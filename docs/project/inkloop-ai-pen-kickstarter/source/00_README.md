...
| `InkLoop_会议纪要后处理最佳实践方案_V2.md` | 商务会议/访谈会后处理 V2 的产品原则、数据契约、目标流程与验收标准 |
| `InkLoop_NoDeskAI_会议接入与自有事实链技术方案_V1.md` | No Desk AI 虚拟摄像头/音频采集方案与 InkLoop 现有系统的差异、整合架构和分阶段落地基线 |
...
3. 商务会议链路研发先读 `InkLoop_会议纪要后处理最佳实践方案_V2.md`，确认短纪要、结构化卡片、OCR 与长会议的目标边界；完整报告已经下线。
4. 涉及 Meet/Zoom 实时板书、macOS 音频事实源和自有转写时，再读 `InkLoop_NoDeskAI_会议接入与自有事实链技术方案_V1.md`。
5. 然后读 `03_各模块技术方案.md` 和 `04_AI与InkGraph数据契约.md`，给研发、硬件、AI 和前端拆任务。
6. 最后读 `05_目标与里程碑_10月底Kickstarter倒排.md`、`06_Kickstarter_GTM与众筹页面方案.md`、`07_风险_验收指标_降级方案.md`，用于执行和周会追踪。
