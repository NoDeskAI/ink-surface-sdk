/**
 * Knowledge Export 上传触发链（交付路线 Y·设备→panel）：collect → build → publish 串成一个动作。
 * mobile-main 的 __inkloop hook 与 设置页「导出知识库快照」按钮共用同一份逻辑（不靠 window 全局·有类型）。
 *
 ...
 * V1 本地 Cloud Hub：userId 默认来自当前设备 session，Obsidian 投影按用户隔离；只有服务端显式配置
 *       INKLOOP_VAULT_FORCE_USER 时才钉死单用户 vault 桶。
 */
import type { VaultExportBundle } from './vault-export';
import type { VaultEntityRef } from './vault-collect';
import { runtimeUserId, sessionDeviceId } from '../../core/auth';

export const VAULT_PUBLISH_USER_ID = 'local_demo'; // V1 本地演示默认 namespace；正式登录后使用当前 session userId。
...
export interface VaultPublishResult {
  ...
  release_hash?: string;
  release_id?: string;
  uploaded_at?: string; // Cloud Hub 接收时间；优先用于 UI 状态，避免设备时钟错误污染导出时间。
...
export async function publishVaultFromDevice(opts: VaultPublishOpts = {}): Promise<VaultPublishResult> {
  const userId = opts.userId ?? runtimeUserId() ?? VAULT_PUBLISH_USER_ID;
  const deviceId = opts.deviceId ?? sessionDeviceId() ?? VAULT_PUBLISH_DEVICE_ID;
...
/**
 * 阶段⑤·按需导出入口：单会议/单书「导出知识库快照」按钮调这个。
...
export async function publishEntityToVault(ref: VaultEntityRef, opts: VaultPublishOpts = {}): Promise<EntityPublishResult> {
  ...
  if (result.ok && ref.mode === 'meeting') {
    ...
    await updateMeeting(ref.meetingId, { exported_at: result.uploaded_at ?? new Date().toISOString() }).catch(() => {});
  }
  return result;
}
