...
describe('runtime sync bridge', () => {
  ...
  it('bridges mark ledger entries to IndexedDB runtime snapshot and outbox exactly once', async () => {
    ...
    const marks = [mark({ mark_id: 'mark_1', seq: 1, pen_down_at: 1_751_500_000_123 })];
    ...
    expect((add?.payload.annotation as { inkloop_mark?: Record<string, unknown> })?.inkloop_mark?.pen_down_at).toBe(1_751_500_000_123);
    await store.close();
  });
