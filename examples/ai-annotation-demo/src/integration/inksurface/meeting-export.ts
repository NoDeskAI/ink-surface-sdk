/**
 * 会议 → InkSurface L1 投影导出（C「打通链路」）。
 *
 * 关键洞察：**会议不需要对方的「会议槽位」**——把会议事件投影进现有「文档 + 标注」契约即可：
 *   · 转写  → 可选 context block（段摘要＝heading 块、每句转写＝paragraph 块；复用 segment.ts 的分段）。
 *   · 手写  → annotation KnowledgeObject，锚到它落在的那一段（heading 块）。
 *   · 会议输出候选 → summary KnowledgeObject。
 * 这样 Obsidian 直接渲染成「会议 source-unit 投影 + 旁注手写」，并进思维图（每条手写/总结 [[链接]]→会议文档枢纽）。
 * 这条导出不是音频自动整理主链路；V1 提升为可信 KnowledgeObject 仍需要板书/墨迹事件证据。
 *
...
import type { RecapSegment } from '../panel-feishu/segment';
import { buildEpaperMeetingTimeline } from '../lark-meeting-timeline/epaper-timeline';
...
import type { KnowledgeKind, KnowledgeObject, NormBBox } from '../../knowledge/knowledge-object';
import {
  buildInkloopDocUri,
...
function structuredMeetingMark(text: string | undefined): { kind: StructuredMeetingKind; body: string; title: string } | null {
  const raw = (text || '').trim();
  if (!raw) return null;
  const match = raw.match(/^(决策|决定|decision|任务|待办|action|todo|风险|risk|问题|疑问|question|q)\s*[:：-]\s*(.+)$/i);
  if (!match) return null;
  const label = match[1].toLowerCase();
  const body = (match[2] || '').trim();
  if (!body) return null;
  if (label === '决策' || label === '决定' || label === 'decision') {
    return { kind: 'meeting_decision', body, title: `Decision: ${body.slice(0, 56)}` };
  }
  if (label === '风险' || label === 'risk') {
    return { kind: 'meeting_risk', body, title: `Risk: ${body.slice(0, 56)}` };
  }
  if (label === '问题' || label === '疑问' || label === 'question' || label === 'q') {
    return { kind: 'qa', body, title: `Question: ${body.slice(0, 56)}` };
  }
  return { kind: 'meeting_action', body, title: `Action: ${body.slice(0, 56)}` };
}

const visualToolOf = (tool: PersistedStroke['tool']): 'pen' | 'highlighter' => (tool === 'highlighter' ? 'highlighter' : 'pen');
const isVisibleStroke = (s: PersistedStroke): boolean => s.tool === 'pen' || s.tool === 'aipen' || s.tool === 'highlighter' || s.tool === 'underline';
...
export async function buildMeetingL1Export(meetingId: string, opts: MeetingExportOpts = {}): Promise<MeetingL1Export> {
  ...
  for (const token of meetingTranscriptCacheTokens(meeting)) {
...
export async function assembleMeetingL1Export(input: MeetingExportInput, opts: MeetingExportOpts = {}): Promise<MeetingL1Export> {
  ...
  // ④ KO：会议输出候选（summary）+ 每笔手写（annotation·锚到所在段 heading 块）。
  ...
  for (const mk of segMarks) {
    ...
    const ko = await finalize({
      ...
      titleOverride: structured?.title,
      body,
