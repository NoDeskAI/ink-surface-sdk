/**
 * Knowledge Export 下载执行器（交付路线 Y · 下载器侧 · I/O 注入·可测）——拿 buildVaultSyncPlan 的计划 + 注入式 I/O 口，
 * 执行「下载写盘 / 删陈旧 / 跳冲突」，并算出**更新后的 sync-state**（写回 .inkloop-sync-state）。
 * Obsidian/Markdown/备份下载器只实现 SyncIoPort（vault.adapter 写 + fetch 拉 blob），调用本执行器 → 难逻辑全测得到。
 * 这是 export-only 链路，不写 Runtime Sync outbox、inbox、cursor 或 sidecar truth。
 *
