...
import { assembleMeetingL1Export, meetingDocId, meetingTranscriptCacheTokens, type MeetingExportInput } from './meeting-export';
...
describe('assembleMeetingL1Export（会议→L1）', () => {
  it('导出读取转写缓存时 fallback 到 Feishu note/docx token', () => {
    expect(meetingTranscriptCacheTokens(meeting({
      feishu_minute_token: undefined,
      feishu_meeting_id: '7659677460199738340',
    }))).toEqual(['feishu_note_docx:7659677460199738340']);

    expect(meetingTranscriptCacheTokens(meeting({
      feishu_minute_token: 'minute_tok',
      feishu_meeting_id: '7659677460199738340',
  ...
  it('显式会议标记前缀会升级为 action/decision/risk/question KO', async () => {
    const out = await assembleMeetingL1Export(input({
      marks: [
        mk('decision', 12, '决策：MVP 演示以 Web 导入到墨水屏标记到 Obsidian 投影为闭环'),
        mk('action', 18, '任务：本周补齐 M103 低延迟手写验收'),
        mk('risk', 24, '风险：T10C Plus 手写延迟还未验证'),
        mk('question', 30, '问题：Obsidian 回跳链接应该打开 Web 还是墨水屏'),
      ],
    }), OPTS);

    expect(out.knowledgeExport.objects.map((k) => k.kind)).toContain('meeting_decision');
    expect(out.knowledgeExport.objects.map((k) => k.kind)).toContain('meeting_action');
    expect(out.knowledgeExport.objects.map((k) => k.kind)).toContain('meeting_risk');
    expect(out.knowledgeExport.objects.map((k) => k.kind)).toContain('qa');
    expect(out.knowledgeExport.objects.find((k) => k.kind === 'meeting_decision')?.body_md).toContain('MVP 演示');
    expect(out.knowledgeExport.objects.find((k) => k.kind === 'qa')?.render_hints?.markdown_callout).toBe('question');
  });
