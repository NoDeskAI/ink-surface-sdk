...
import { koId, markLocationObjectRefs } from '../../knowledge/builder';
...
function isRuntimeSilentMark(mark: PersistedMark): boolean {
...
function resolveMarkBlock(mark: PersistedMark, samePage: ProjectionBlock[]): ProjectionBlock | null {
  const runs = markLocationObjectRefs(mark);
...
function resolveKoBlock(ko: KnowledgeObject, samePage: ProjectionBlock[], markById: ReadonlyMap<string, PersistedMark>): ProjectionBlock | null {
  ...
  const markRuns = [...new Set((ko.provenance.mark_ids ?? [])
    ...
    .flatMap(markLocationObjectRefs))];
...
const visualToolOf = (tool: PersistedStroke['tool']): 'pen' | 'aipen' | 'highlighter' | 'underline' => {
  if (tool === 'aipen') return 'aipen';
...
function strokesToVisual(strokes: PersistedStroke[], color: string, blockBBox: NormBBox): RuntimeVisualStroke[] {
  return strokes
    .filter((s) => s.tool === 'pen' || s.tool === 'aipen' || s.tool === 'highlighter' || s.tool === 'underline') // eraser/hand 非可视笔
    .map((s) => ({
...
const isVisibleStroke = (s: PersistedStroke): boolean =>
  s.tool === 'pen' || s.tool === 'aipen' || s.tool === 'highlighter' || s.tool === 'underline';
...
export async function buildRuntimeAndVisual(
  ...
  const silentMarkIds = new Set(allMarks.filter(isRuntimeSilentMark).map((mark) => mark.mark_id));
  const exportKos = exportableKos.filter((ko) => !(ko.provenance.mark_ids ?? []).some((id) => silentMarkIds.has(id)));
  ...
  const marks = allMarks.filter((m) => !m.is_tombstone && !isRuntimeSilentMark(m));
