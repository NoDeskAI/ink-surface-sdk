/**
 * Knowledge Export 下载计划器（交付路线 Y · 消费端/下载器侧 · 纯函数）——给远端 release manifest + 本地同步状态，
 * 算出「下载哪些 / 删哪些 / 哪些是用户改过的冲突 / 哪些没变跳过 / 哪些路径非法拒掉」。
 * 下载器（Obsidian/Markdown/备份目标）只围着这个纯核做 I/O（拉 blob / 写盘 / 删文件）→ 难的判定全在这测得到。
 * 这是 export-only 链路，不参与 Runtime Sync 的 outbox/inbox/cursor。
 *
