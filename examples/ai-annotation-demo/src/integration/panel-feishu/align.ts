...
export interface TranscriptCue {
  ...
  speakerSource?: FeishuTranscriptSpeakerSource;
  speakerDeviceOwner?: string;
...
function splitEmbeddedSpeakerMarkers(input: {
  startMs: number;
  endMs: number;
  rawText: string;
}): { cues: Array<Omit<TranscriptCue, 'index'>>; hadMarker: boolean } {
  const outer = splitSpeaker(input.rawText);
  const cues: Array<Omit<TranscriptCue, 'index'>> = [];
  let hadMarker = false;
  let current: {
    startMs: number;
    speaker?: string;
    ...
    speaker: outer.speaker,
    speakerSource: outer.speaker ? 'identified' : undefined,
    speakerDeviceOwner: undefined,
    lines: [],
  };

  const flush = (endMs: number): void => {
    const text = current.lines.join('\n').trim();
    if (!text || endMs <= current.startMs) return;
    cues.push({
      startMs: current.startMs,
      endMs,
      speaker: current.speaker,
      ...(current.speakerSource ? { speakerSource: current.speakerSource } : {}),
      ...(current.speakerDeviceOwner ? { speakerDeviceOwner: current.speakerDeviceOwner } : {}),
      text,
      rawText: current.speaker ? `${current.speaker}：${text}` : text,
    });
  };

  for (const line of outer.text.split('\n')) {
    ...
    hadMarker = true;
    const markerStart = Math.max(input.startMs, Math.min(input.endMs, marker.startMs));
    flush(markerStart);
    current = {
      startMs: markerStart,
      speaker: marker.speaker,
      speakerSource: marker.source,
      speakerDeviceOwner: marker.deviceOwner,
      lines: [],
    };
  }
  flush(input.endMs);

  return { cues, hadMarker };
}
...
export function parseSrtTranscript(input: string): TranscriptCue[] {
  ...
  for (const block of blocks) {
    ...
    const split = splitEmbeddedSpeakerMarkers({ startMs, endMs, rawText });
    hadEmbeddedMarker ||= split.hadMarker;
    if (split.cues.length) {
      split.cues.forEach((cue, partIndex) => cues.push({ ...cue, index: partIndex === 0 ? index : 0 }));
    }
  }
  return hadEmbeddedMarker ? cues.map((cue, index) => ({ ...cue, index: index + 1 })) : cues;
}
