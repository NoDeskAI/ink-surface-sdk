...
describe('parseSrtTranscript', () => {
  ...
  it('兼容旧缓存：把正文里的设备说话人标记拆成独立 cue 且不展示系统文案', () => {
    const legacy = `1
00:01:32,000 --> 00:01:44,000
王辰炜：还是跟之前一样，哲帆先同步一下进度吧。
...
金哲帆：OK，好的。`;
    ...
    const cues = parseSrtTranscript(legacy);

    expect(cues).toHaveLength(3);
    expect(cues[0]).toMatchObject({ speaker: '王辰炜', startMs: 92_000, endMs: 95_000 });
    expect(cues[1]).toMatchObject({
      speaker: '说话人 8',
      speakerSource: 'device_diarization',
      speakerDeviceOwner: '王辰炜',
      startMs: 95_000,
      endMs: 104_000,
      text: '好，那个。',
    });
    expect(cues[2]).toMatchObject({ speaker: '金哲帆', text: 'OK，好的。' });
