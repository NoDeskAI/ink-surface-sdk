...
import { clearOsdInkAfterCommit, forceResetOsdInkNow, isPenDown } from './m103-input-source';
...
function visibleCanvas(): VisibleCanvas | null {
  ...
  if (document.body.dataset.read === 'books') return null;
  if (state.tool !== 'pen' && state.tool !== 'aipen') return null;
  ...
  for (const el of nodes) {
    ...
    if (r.width > 0 && r.height > 0) {
      const selector = el.matches('#ink-layer') ? '#ink-layer' : el.matches('#reader') ? '#reader' : '.reader-ink';
...
function contentSig(canvas = visibleCanvas()): string {
...
function maybeClearOsd(): void {
  ...
  const sig = contentSig(canvas);
  if (sig === lastClearSig) return;       // 内容身份/位置/遮挡都没变(如写字后 mark 重绘)→别清，避免每笔清=闪
  lastClearSig = sig;
  if (canvas) clearOsdInkAfterCommit();   // 双 rAF 屏障：等新内容(redrawInk)画完再清 OSD
  else forceResetOsdInkNow();             // 书架/浮层/离开阅读面没有 canvas 承接，必须强清硬件 OSD，不能走可能 no-op 的普通 clear
}
...
/** 明确离开可写面时调用：马上让原生画区变成 null。
 *  MutationObserver 会兜底，但返回书架/刷新列表这种路径不能等下一轮 DOM 观察，否则 OSD 会短暂盖在书架上。 */
export function disarmM103HqHwAreaNow(): void {
  lastClearSig = '';
  forceResetOsdInkNow();
  const ch = channel();
  if (!ch) return;
  try { ch.postMessage('null'); } catch { /* no-op */ }
}
