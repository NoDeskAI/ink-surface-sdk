...
export function inkLoopDeviceProfile(): string | null {
  const w = window as unknown as { __inkloopDeviceProfile?: string };
  return w.__inkloopDeviceProfile ?? null;
}

export function isOnyxPaperDevice(): boolean {
  const profile = inkLoopDeviceProfile();
  return profile === 'onyx-t10' || profile === 'onyx-boox';
}

export function supportsNativeInputSource(): boolean {
  return isM103Device() || isOnyxPaperDevice();
}
