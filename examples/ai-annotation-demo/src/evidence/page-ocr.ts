...
/** 扫描页判定：PDF/article 页没有任何可用文字层 run，就允许整页 OCR。
 *  有些扫描 PDF 不是单纯 paintImageXObject，大图区域抽取可能为空；不能因此跳过 OCR。 */
export function isScannedPage(): boolean {
  return state.surfaceType === 'article' && state.textBlocks.length === 0;
}
...
function textToApproxBlocks(text: string, pageIndex: number): OcrTextBlock[] {
  const lines = text
    .split(/\r?\n+/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  const top = 0.08;
  const usableH = 0.84;
  const lineH = Math.min(0.04, usableH / Math.max(lines.length, 1));
  return lines.map((line, i) => ({
    id: `ocr_flat_${pageIndex}_${i}`,
    text: line,
    bbox: [0.08, top + i * lineH, 0.84, Math.max(0.018, lineH * 0.82)] as NormBBox,
    confidence: 0.35,
    language: /[一-鿿]/.test(line) ? 'zh' : 'en',
  }));
}
...
export async function ensureScannedPageLayer(pageId: string | null): Promise<void> {
  ...
  try {
    ...
    if (blocks.length) {
      ...
      await flatFallback(pageId, img, pageIndex); // 没出框 → 退 Phase 1
    }
  } catch {
    await flatFallback(pageId, img, pageIndex);    // ocr-layout 不可用（生产无此端点）→ 退 Phase 1
  } finally {
...
async function flatFallback(pageId: string, img: string, pageIndex: number): Promise<void> {
  ...
  try {
    ...
    if (text) {
      flatCache.set(pageId, text);
      const blocks = textToApproxBlocks(text, pageIndex);
      if (blocks.length) {
        layerCache.set(pageId, blocks);
        if (state.pageId === pageId) applyLayerIfReady(pageId);
      }
    }
    const ms = Math.round(performance.now() - t0);
    devEmit('pageocr', () => ({ pageId, page_index: pageIndex, phase: text ? 'flat' : 'none', source, chars: text.length, latency_ms: ms, sample: text.slice(0, 60) }));
  } catch { /* 静默：下次渲染再试 */ }
}
