...
import { bus, dropOverlay, state, settings } from '../app/state';
...
function openPop(o: ScreenOverlay): void {
  ...
  mk('原文', () => { bus.emit('reader:source-focus', o); closePop(); });
  ...
  popEl = pop;
  const [x, y, , h] = o.geometry.anchor_bbox;
...
function add(o: ScreenOverlay): void {
  ...
  acts.append(
    mkBtn('原文', () => bus.emit('reader:source-focus', o)),
