...
interface OnyxEpdChannel { refreshDirtyRect?(x: number, y: number, width: number, height: number): boolean; }
...
function onyxEpdChannel(): OnyxEpdChannel | null {
  const w = window as unknown as { InkLoopOnyxEpd?: OnyxEpdChannel };
  return w.InkLoopOnyxEpd ?? null;
}

/** 原生桥是否在场且启用（web/dev 恒为 false）。 */
export function einkAvailable(): boolean { return enabled && (!!channel() || !!onyxEpdChannel()?.refreshDirtyRect); }
...
function flushInkDirty(): void {
  ...
  const onyx = onyxEpdChannel();
  if (!onyx?.refreshDirtyRect) return;
  const dpr = window.devicePixelRatio || 1;
  const vw = window.innerWidth || 1, vh = window.innerHeight || 1;
  const pad = 6 * dpr;
  const x = Math.floor(p.x0 * vw * dpr - pad);
  const y = Math.floor(p.y0 * vh * dpr - pad);
  const w = Math.ceil((p.x1 - p.x0) * vw * dpr + pad * 2);
  const h = Math.ceil((p.y1 - p.y0) * vh * dpr + pad * 2);
  try { onyx.refreshDirtyRect(x, y, w, h); } catch { /* no-op */ }
}
/** 通用 A2 脏区核心：并集视口归一化矩形[0,1]，短窗后推一帧局部快刷。夹值 + 丢空矩形。 */
function pushDirty(x0: number, y0: number, x1: number, y1: number): void {
  if (!enabled || (!channel() && !onyxEpdChannel()?.refreshDirtyRect)) return;
  ...
  inkTimer = window.setTimeout(flushInkDirty, 60);
}
