...
import type { PersistedReflowArtifact } from '../surface/reflow-artifact';
...
export interface PersistedStroke {
  tool: 'pen' | 'aipen' | 'highlighter' | 'underline' | 'eraser' | 'hand';
...
export interface PersistedPage {
  ...
  reflow_artifact?: PersistedReflowArtifact | null; // v1 派生 artifact 元数据；旧 reflow 字段仍保留作兼容缓存
...
export type PersistedLibrarySyncStatus = 'local' | 'syncing' | 'synced' | 'failed' | 'cloud_only';
export type PersistedLibrarySource = 'web' | 'paper_wifi' | 'paper_file' | 'cloud';

/**
 * Cloud Hub 与本地 Library 的最小同步索引。
 * 源文件字节仍放 pdf_blobs；这里记录云端 manifest、同步状态、失败重试信息。
 */
export interface PersistedLibrarySync {
  document_id: string;
  ...
  sync_status: PersistedLibrarySyncStatus;
...
export interface PersistedMark extends BaseEntry {
  ...
  tool: 'pen' | 'highlighter' | 'underline';      // 代表工具（自 event_type 派生）
...
/** 飞书官方智能纪要原文：来自会议 note artifact_type=1/docx raw_content，和 InkLoop 自己的总结分开存放。 */
export interface FeishuNoteSummaryRecord {
...
export interface PersistedMeetingTranscriptSegment {
  index: number;
  speaker: string;
...
export interface PersistedMeetingMinute {
  ...
  srt: string;                      // 规范化 SRT：系统说话人标记不混入正文
  parser_version?: number;
  source_raw_content?: string;      // 飞书 DOCX 原文，追溯用；UI/AI 不直接消费
  source_segments?: PersistedMeetingTranscriptSegment[];
  title?: string;
  duration_ms?: number;
