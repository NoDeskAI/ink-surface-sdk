...
export interface AnnotationCaptureContext {
...
/** 单笔封装为契约 shape。会话内多笔共享一个 trace_id（决策：停笔会话）。 */
export function makeEvent(stroke: Stroke, traceId: string, capture: AnnotationCaptureContext = {}): AnnotationEvent | null {
  const documentId = capture.documentId || state.documentId;
  const pageId = capture.pageId || state.pageId;
  ...
  return {
    ...
    page_id: pageId,
    page_index: typeof capture.pageIndex === 'number' ? capture.pageIndex : state.pageIndex,
    ...(capture.contextId ? { context_id: capture.contextId } : {}),
    event_type: stroke.tool === 'highlighter' ? 'highlight' : stroke.tool === 'underline' ? 'underline' : classify(stroke.points, bb),
    geometry: { bbox: bb },
...
export function recordEvent(stroke: Stroke, traceId: string, pointerType: string, penUpAt: number, capture: AnnotationCaptureContext = {}): AnnotationEvent | null {
  const evt = makeEvent(stroke, traceId, capture);
  if (!evt) return null;
