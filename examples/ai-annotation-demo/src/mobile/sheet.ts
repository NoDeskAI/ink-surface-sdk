...
export function confirmSheetWithOption(opts: {
  title?: string;
  message: string;
  confirm?: string;
  ...
  return new Promise((resolve) => {
    const h = mountSheet(
      ...
      `<p class="msheet-msg">${esc(opts.message)}</p>`
      + `<label class="msheet-check">`
      + `<input class="msheet-check-input" type="checkbox"${opts.option.checked ? ' checked' : ''} />`
      + `<span class="msheet-check-box" aria-hidden="true"></span>`
      + `<span class="msheet-check-copy"><span class="msheet-check-title">${esc(opts.option.label)}</span>`
      + (opts.option.hint ? `<span class="msheet-check-hint">${esc(opts.option.hint)}</span>` : '')
      + `</span></label>`,
      opts.confirm ?? '确定',
    );
    const input = h.body.querySelector<HTMLInputElement>('.msheet-check-input');
    wire(h, () => resolve({ confirmed: false, checked: !!input?.checked }), () => {
      resolve({ confirmed: true, checked: !!input?.checked });
    });
  });
}
