...
const CORE: SCtl[] = [
  { label: 'AI 触发模式', desc: 'AI笔触=只选中 AI 笔写的进 AI，且直接回应；普通笔只做阅读标记。自动判意=每笔都判(实验)', badge: 'eff', effect: 'changed', type: 'select', opts: [['pen', 'AI 笔触（显式回应·默认）'], ['auto', '自动判意（每笔都判·实验）']], get: () => settings.aiTrigger, set: (v) => { settings.aiTrigger = v as typeof settings.aiTrigger; } },
  ...
  { label: 'PDF 优化引擎', badge: 'eff', effect: 'changed', type: 'select', opts: [['local', '规则版面（保留原文）']], get: () => 'local', set: () => { settings.reflowProvider = 'local'; } },
  { label: 'PDF 优化模型（legacy）', badge: 'eff', effect: 'changed', type: 'select', opts: [['gemini-3.1-flash-lite', 'gemini-3.1-flash-lite'], ['gemini-3.5-flash', 'gemini-3.5-flash'], ['kimi-k2.6', 'kimi-k2.6'], ['claude-sonnet-4-6', 'claude-sonnet-4-6']], get: () => settings.reflowModel, set: (v) => { settings.reflowModel = v; } },
  { label: 'PDF 优化急算（翻页后台）', badge: 'eff', effect: 'save', type: 'check', get: () => settings.reflowEager, set: (_v, c) => { settings.reflowEager = c; } },
...
const DEBUG: SCtl[] = [
  { label: '网络线路（所有云服务共用出口）', desc: '换地方免重打包一键切·对后续请求即时生效。默认=构建烧录值；本地 debug 包的内网直连会使用构建时烧录的 Mac LAN 地址；公网=cloudflared。AI/Obsidian上传/飞书妙记全走这一个出口。', badge: 'dev', effect: 'changed', type: 'select', opts: [['', '默认（构建烧录值）'], ['cloud', '公网 cloudflared'], ['intranet', '内网直连']], get: () => apiRouteChoice(), set: (v) => setApiRoute(v) },
  { label: 'AI 笔 legacy 圈选开关', desc: '兼容旧实验开关；V1 主链路中 AI 笔已经默认直接回应。', badge: 'dev', effect: 'changed', type: 'check', get: () => settings.aiPenExplicit, set: (_v, c) => { settings.aiPenExplicit = c; } },
  { label: '设备遥测（给开发者）', desc: '开后事实流推 window.__devtel + console（让 Claude 经 CDP 直读板上 gesture/识别/分类/推理/阅读优化）', badge: 'dev', effect: 'changed', type: 'check', get: () => settings.devtel, set: (_v, c) => { settings.devtel = c; } },
...
const WEAK: SCtl[] = [
  { label: '导入后生成 PDF 优化缓存', desc: '仅下次导入 PDF 生效，默认关', badge: 'weak', effect: 'save', type: 'check', get: () => settings.preprocess.reflowEnabled, set: (_v, c) => { settings.preprocess.reflowEnabled = c; } },
  { label: 'PDF 优化缓存页数', badge: 'weak', effect: 'save', type: 'number', min: 0, max: 100, get: () => settings.preprocess.reflowPages, set: (v) => { settings.preprocess.reflowPages = Math.min(100, Math.max(0, parseInt(v, 10) || 0)); } },
];
...
export function renderSettings(): void {
  ...
  const body =
    ...
    + `<div class="sgrp-h">知识导出 · Markdown</div>`
    + `<div class="srow"><div class="sl"><div class="sn">导出知识库快照 <span class="sbadge eff">生效</span></div><div class="sd">生成干净 Markdown release 发到 panel，供 Obsidian/备份目标显式拉取；实时书写同步走 Runtime Sync。</div></div><button type="button" class="hbtn" id="dv-vault-pub"${vaultPublishBusy ? ' disabled' : ''}>${vaultPublishBusy ? '导出中…' : '导出'}</button></div>`
  ...
  // 防导出中其它 bus 事件触发 renderSettings 把捕获的 status DOM 变 detach（并发防重入另有 publishVaultFromDevice 内 controller）。
...
function diagHtml(): string {
  ...
  return `<div class="dl"><b>坐标自测</b><span>${coord} · zoom ${Math.round(state.zoom * 100)}%</span></div>`
    + `<div class="dl"><b>PDF 优化缓存</b><span>${esc(preprocessText)}</span></div>`
...
/** 设置页诊断块原地刷新（metrics/PDF 优化缓存是 live 流，不整页重渲）。 */
...
export function initMobileDev(): void {
  ...
  // 设置页诊断 live 流（metrics/PDF 优化缓存·只刷诊断块、不整页重渲）
  ...
  bus.on('preprocess:progress', (i, n) => { preprocessText = `生成 PDF 优化缓存 ${Number(i)}/${Number(n)} 页…`; liveDiag(); });
  bus.on('preprocess:done', () => { preprocessText = 'PDF 优化缓存完成'; liveDiag(); });
}
