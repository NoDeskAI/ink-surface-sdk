...
import type { ScreenOverlay, AnnotationEvent, EventType, NormBBox } from '../core/contracts';
import { initInk, redrawInk, type StrokeCaptureContext } from '../capture/ink';
...
const CONTENT_LINE_MARK_QUICK_MS = 4500; // 连续划相邻两行时，换行/挪笔会比写字慢；仅线状内容标记使用
...
const CONTENT_PEN_QUIET_MS = 1800;
let contentEvents: AnnotationEvent[] = [];
let contentStrokes: Stroke[] = [];
let contentRawRefs: (RawRef | undefined)[] = [];
let contentBbox: NormBBox | null = null;
let contentNearBbox: NormBBox | null = null;
let contentNearPad = REGION_NEAR;
let contentLastAt = 0;
...
function nearRegion(bb: NormBBox, pad = REGION_NEAR): boolean {
  ...
  return bboxNearRegion(regNearBbox, bb, pad);
}
function bboxNearRegion(region: NormBBox, bb: NormBBox, pad = REGION_NEAR): boolean {
  const cx = bb[0] + bb[2] / 2, cy = bb[1] + bb[3] / 2;
  return cx >= region[0] - pad && cx <= region[0] + region[2] + pad
    && cy >= region[1] - pad && cy <= region[1] + region[3] + pad;
}
function isLineLikeContentStroke(evt: AnnotationEvent): boolean {
  const bb = nearBoxOf(evt);
  return evt.event_type === 'underline' || (bb[2] >= 0.035 && bb[2] >= bb[3] * 3.5);
}
/**
...
function emitRecordedMark(entry: MarkEntryDraft): void {
  bus.emit('mark:recorded', entry);
}

function enqueueContentMark(entry: MarkEntryDraft): void {
  contentMarkWrites = contentMarkWrites.catch(() => undefined).then(async () => {
    await appendMarkEntry(entry);
    emitRecordedMark(entry);
    bus.emit('mark:resolved', { feature: entry.feature_type, text: entry.marked_text || '' });
  });
}
...
function expandedTextHitBbox(bbox: NormBBox, tool: Stroke['tool']): NormBBox {
  ...
  const padBottom = tool === 'underline' ? 0.028 : 0.012;
  const x = Math.max(0, bbox[0] - padX);
  const y = Math.max(0, bbox[1] - padTop);
  const x2 = Math.min(1, bbox[0] + bbox[2] + padX);
  const y2 = Math.min(1, bbox[1] + bbox[3] + padBottom);
...
function sliceBlockTextByX(text: string, block: NormBBox, x0: number, x1: number): string {
...
function markedTextForTextTool(bbox: NormBBox, tool: Stroke['tool']): string {
  const padX = tool === 'underline' ? 0.01 : 0.006;
  const x0 = Math.max(0, bbox[0] - padX);
  const x1 = Math.min(1, bbox[0] + bbox[2] + padX);
  ...
  const blocks = state.textBlocks
    .map((block, index) => {
      ...
      const tolerance = Math.max(0.006, lineH * (tool === 'underline' ? 0.56 : 0.62));
      const hOverlap = horizontalOverlap(x0, x1, block.bbox);
      return { block, index, dy: Math.abs(strokeMidY - centerY), tolerance, hOverlap };
    })
    .filter((item) => item.dy <= item.tolerance && item.hOverlap > Math.max(0.002, item.block.bbox[2] * 0.02))
    .sort((a, b) => (a.block.bbox[1] - b.block.bbox[1]) || (a.block.bbox[0] - b.block.bbox[0]) || (a.index - b.index));
  const candidates = blocks.length
    ? blocks
    : state.textBlocks
      .map((block, index) => ({ block, index, area: overlapArea(expandedTextHitBbox(bbox, tool), block.bbox), hOverlap: horizontalOverlap(x0, x1, block.bbox) }))
      .filter((item) => item.area > 0 && item.hOverlap > 0)
...
function markedTextForPenMarkupBbox(bbox: NormBBox): string {
  return markedTextForPenMarkupBboxFromBlocks(state.textBlocks, bbox);
}
...
function persistContentStroke(evt: AnnotationEvent, stroke: Stroke): void {
  ...
  enqueueContentMark({
    ...
    bbox: evt.geometry.bbox, tool: isHi ? 'highlighter' : isUnderline ? 'underline' : 'pen',
    coord_space: evt.coord_space ?? 'page_norm',
    capture_surface: evt.capture_surface ?? (evt.pointer_type === 'reader' ? 'reader' : 'page'),
    ...
    scored_type: isHi ? 'highlight' : isUnderline ? 'underline' : 'stroke',
    scored_score: anchoredTextTool ? 1 : 0,
    hmp: null, marked_text: markedText, is_tombstone: false,
    ai_eligible: false, origin: isHi ? 'highlighter' : isUnderline ? 'underline' : 'pen',
...
function resetContentPenAssembly(): void {
  window.clearTimeout(contentTimer);
  contentEvents = [];
  contentStrokes = [];
  contentRawRefs = [];
  contentBbox = null;
  contentNearBbox = null;
  contentNearPad = REGION_NEAR;
  contentLastAt = 0;
}

function contentPenQuietMs(): number {
  return Math.max(CONTENT_PEN_QUIET_MS, settings.regionQuietMs || 1000);
}

function recomputeContentPenAssembly(): void {
  contentBbox = contentEvents.length ? contentEvents.map((e) => e.geometry.bbox).reduce(unionBb) : null;
  contentNearBbox = contentEvents.length ? contentEvents.map((e) => nearBoxOf(e)).reduce(unionBb) : null;
  contentNearPad = contentEvents.reduce((m, e) => Math.max(m, nearPadOf(e)), REGION_NEAR);
  contentLastAt = contentEvents.length ? contentLastAt : 0;
  if (!contentEvents.length) window.clearTimeout(contentTimer);
}

function flushContentPen(reason: FlushReason = 'manual'): void {
  window.clearTimeout(contentTimer);
  const events = contentEvents, strokes = contentStrokes, rawRefs = contentRawRefs;
  resetContentPenAssembly();
  if (!events.length || !strokes.length) return;
  const first = events[0];
  const bookId = first.document_id || state.documentId || 'book';
  const markId = first.event_id;
  const bbox = events.map((e) => e.geometry.bbox).reduce(unionBb);
  const surfacePoints = events.flatMap((e) => e.reflow_ink_points ?? []);
  ...
  const entry: MarkEntryDraft = {
    document_id: bookId, page_id: first.page_id, page_index: typeof first.page_index === 'number' ? first.page_index : pageIdxOf(first.page_id), mark_id: markId,
    strokes: events.map((e, i) => persistedStrokeOf(e, strokes[i])),
    bbox, tool: 'pen', color: '#1A1A1A',
    coord_space: first.coord_space ?? 'page_norm',
    ...
    kind: strokeCount >= 2 ? 'handwriting' : undefined,
    kind_source: strokeCount >= 2 ? 'local_group' : undefined,
    scored_type: strokeCount >= 2 ? 'stroke_group' : 'stroke',
    scored_score: strokeCount >= 2 ? 0.8 : 0,
    hmp: null,
    marked_text: strokeCount >= 2 ? `手写 ${strokeCount} 笔` : '',
    raw_ref, is_tombstone: false,
    ai_eligible: false, origin: 'pen',
    ...(reflowAnchorRuns.length ? { reflow_anchor_runs: reflowAnchorRuns } : {}),
  };
  enqueueContentMark(entry);
  gtrace({ page_id: first.page_id, contentPenGroup: { strokes: strokeCount, mark: markId, reason, bbox: bbox.map((n) => +n.toFixed(4)) } });
  if (strokeCount === 1) void maybeResolveContentMarkup(first, strokes[0], markId).catch(() => undefined);
}

function ingestContentPen(evt: AnnotationEvent, stroke: Stroke): void {
  const nb = nearBoxOf(evt);
  const now = performance.now();
  const gapMs = contentLastAt ? now - contentLastAt : 0;
  const evtPad = nearPadOf(evt);
  const joinPad = evt.near_bbox && (!contentLastAt || gapMs <= REGION_READER_QUICK_MS)
    ? Math.max(contentNearPad, evtPad)
    : REGION_NEAR;
  const currentHasLineLike = contentEvents.some(isLineLikeContentStroke);
  const nextIsLineLike = isLineLikeContentStroke(evt);
  const quickMs = currentHasLineLike && nextIsLineLike
    ? Math.max(REGION_READER_QUICK_MS, CONTENT_LINE_MARK_QUICK_MS)
    : REGION_READER_QUICK_MS;
  if (contentNearBbox && !shouldJoinContentPenStroke({
    region: contentNearBbox,
    bbox: nb,
    pad: joinPad,
    gapMs,
    quickMs,
    currentHasLineLike,
    nextIsLineLike,
  })) flushContentPen('far-stroke');
  contentEvents.push(evt);
  contentStrokes.push(stroke);
  contentRawRefs.push(settings.bedrock ? bedrockMarkBoundary(evt.document_id || state.documentId || 'book') : undefined);
  contentBbox = contentBbox ? unionBb(contentBbox, evt.geometry.bbox) : evt.geometry.bbox;
  contentNearBbox = contentNearBbox ? unionBb(contentNearBbox, nb) : nb;
  contentNearPad = Math.max(contentNearPad, evtPad);
  contentLastAt = now;
  window.clearTimeout(contentTimer);
  contentTimer = window.setTimeout(() => flushContentPen(`quiet-${contentPenQuietMs()}ms`), contentPenQuietMs());
}

function persistOrIngestContentStroke(evt: AnnotationEvent, stroke: Stroke): void {
  if (stroke.tool === 'pen') ingestContentPen(evt, stroke);
  else persistContentStroke(evt, stroke);
}
...
export function flushRegion(reason: FlushReason = 'manual', diag: Record<string, number> | null = null): void {
  if (reason === 'manual' || reason === 'view-switch') flushContentPen(reason);
...
function ingestStroke(evt: AnnotationEvent, stroke: Stroke): void {
  ...
  regRawRefs.push(settings.bedrock ? bedrockMarkBoundary(evt.document_id || state.documentId || 'book') : undefined);
...
async function maybeResolveContentMarkup(evt: AnnotationEvent, stroke: Stroke, markId: string): Promise<void> {
  ...
  if (!deliberateCircleSize(evt.geometry.bbox, charH) || evt.stroke_points.length < 8) return; // 字级小圈（口/日/O）：是书写不是圈选
  const lineLikePenStroke = stroke.tool === 'pen' && isLineLikePenMarkupBbox(evt.geometry.bbox);
  const broadMarkedText = stroke.tool === 'pen' ? markedTextForPenMarkupBbox(evt.geometry.bbox) : '';
  const lineLikePenText = lineLikePenStroke && !!broadMarkedText;
  const circleCandidate = !lineLikePenStroke && s.type === 'circle' && s.score >= 0.45;
  if (!circleCandidate && !broadMarkedText) return; // 普通线条/涂鸦不补语义
  const baseGeom = classifyStrokeFeature([s], [evt.geometry.bbox], evt.stroke_points, evt.geometry.bbox, charH, clsDim, clsDim);
  if (circleCandidate && (baseGeom.type !== 'markup' || baseGeom.raw.templateType !== 'circle')) return; // 非圈模板（并批降级等）：不是圈选手势
  const fallbackTemplateType: EventType = lineLikePenText ? 'underline' : 'circle';
  const geom = circleCandidate
    ? baseGeom
    : {
        ...baseGeom,
        type: 'markup' as const,
        confidence: Math.max(baseGeom.confidence, 0.5),
        raw: {
          ...baseGeom.raw,
          templateType: fallbackTemplateType,
          templateScore: Math.max(baseGeom.raw.templateScore, 0.46),
        },
      };
  if (state.pageId !== pid) return; // 已翻页：取证上下文不在了，放弃（内容 mark 本体已完好落库）
  let hmp: Awaited<ReturnType<typeof captureMark>>['hmp'] | null = null;
  let markedText = broadMarkedText;
  let featureType: MarkEntryDraft['feature_type'] = 'markup';
  let featureConfidence = lineLikePenText ? 1 : 0.5;
  let kind: string | undefined = lineLikePenText ? 'markup' : undefined;
  let kindSource: string | undefined = lineLikePenText ? 'local_text_line' : undefined;
  let scoredType = lineLikePenText ? 'underline' : 'circle';
  let scoredScore = lineLikePenText ? Math.max(s.score, 0.8) : Math.max(s.score, 0.46);
  if (!lineLikePenText) {
    const repr: AnnotationEvent = {
      ...evt, event_type: circleCandidate ? s.type : 'circle',
    ...
    hmp = cap.hmp;
    markedText = cap.markedText.trim() || (hmp?.text_hint ?? '').trim() || broadMarkedText;
    featureType = cap.feature.type;
    featureConfidence = cap.feature.confidence;
    kind = cap.kind;
    kindSource = cap.kindSource;
    scoredType = circleCandidate ? s.type : 'circle';
    scoredScore = circleCandidate ? s.score : Math.max(s.score, 0.46);
  }
  if (!markedText) {
  ...
  const revision: Omit<PersistedMark, 'entry_id' | 'seq' | 'created_at'> = {
    ...rest,
    feature_type: featureType, feature_confidence: featureConfidence,
    kind, kind_source: kindSource,
    scored_type: scoredType, scored_score: scoredScore,
  ...
  await appendMarkEntry(revision);
  emitRecordedMark(revision);
...
async function resolveRegion(batch: AnnotationEvent[], strokes: Stroke[], flushInfo: Record<string, unknown> = {}, rawRefs: (RawRef | undefined)[] = []): Promise<void> {
  ...
  const bookId = batch[0].document_id || state.documentId || 'book';
  ...
  // AI 笔或 dev 显式圈选开关都会尝试把圈内普通墨迹识别成所标内容。
  // 普通笔仍是纯阅读标记；只有 AI 笔进入回应链路。
  ...
  const tool: 'pen' | 'highlighter' | 'underline' = realStrokes.some((s) => s.tool === 'highlighter')
    ? 'highlighter'
    : realStrokes.some((s) => s.tool === 'underline')
      ? 'underline'
      : 'pen';
  ...
  // V1 产品态：AI 笔是用户明确写给 AI 的命令，必须回应，不再交给 fold/respond 分类器吞掉。
  // 普通笔在 auto 模式下仍可走分类器；默认 pen 模式下普通笔不会进入这里。
  if (isAiPen) void commitSession(bookId, 'handwriting', mark, true);
...
export function wireAnnotationLoop(inkLayer: HTMLCanvasElement): void {
  const recordCapturedEvent = (stroke: Stroke, traceId: string, pointerType: string, penUpAt: number, capture: StrokeCaptureContext): AnnotationEvent | null =>
    recordEvent(stroke, traceId, pointerType, penUpAt, {
  ...
  initInk(inkLayer, (stroke, pointerType, penUpAt, capture) => {
    ...
    if (!strokeEntersAI(stroke)) {
      const evt = recordCapturedEvent(stroke, shortId('trc'), pointerType, penUpAt, capture);
      if (evt) persistOrIngestContentStroke(evt, stroke);
      return;
    }
    if (!sessionTrace) sessionTrace = shortId('trc'); // 一段区域的笔共享 trace（recordEvent 要打点/计延迟，原版页专属）
    const evt = recordCapturedEvent(stroke, sessionTrace, pointerType, penUpAt, capture);
    if (!evt) return;
  ...
  bus.on('reader:gesture', (p) => {
    ...
    else persistOrIngestContentStroke(event, stroke);
  });
  ...
  bus.on('overlay:state', (o) => {
    ...
    if (docId) void updateOverlayState(docId, ov).then(() => bus.emit('aiturn:appended', docId));
  });
  ...
  bus.on('stroke:cancel', (s) => {
    ...
    const ci = contentStrokes.indexOf(stroke);
    if (ci >= 0) {
      contentStrokes.splice(ci, 1);
      contentEvents.splice(ci, 1);
      contentRawRefs.splice(ci, 1);
      recomputeContentPenAssembly();
    }
