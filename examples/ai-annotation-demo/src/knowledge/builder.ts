/**
 * 旧阅读投影 builder —— Tier 2 语义账本（marks + ai_turns）→ KnowledgeObject 的**适配器/投影**。
 *
 * 数据架构总纲：同一真相 → 适配器 → 各消费方。这是"知识笔记"投影：把账本折叠成对外 KO，
 * 交给协作方的 Obsidian/Notion 适配器。它不是 AI Pen Kickstarter V1 的 source-of-truth
 * 主链路；V1 LessonGraph / MeetingGraph 的知识沉淀走 packages/knowledge-schema。**不改任何真相 schema、不碰基岩**——纯派生、每次现算、不落库。
 *
...
import { sha256Hex as schemaSha256Hex, type EntityMembership, type KnowledgeEntity, type KoRelationGroup } from 'ink-surface-sdk/knowledge-schema';
import {
  KO_SCHEMA_VERSION,
  type KnowledgeKind,
...
async function sha256Bytes(s: string): Promise<Uint8Array> {
  const hex = await schemaSha256Hex(s);
...
export async function finalize(d: Draft): Promise<KnowledgeObject> {
  ...
  const ko: KnowledgeObject = {
    ...
    source: {
      ...
      inkloop_uri: inkloopUri(d.documentId, d.pageIndex, d.uriAnchor ?? d.objectRefs[0]),
    },
...
function isReviewLaterMark(m: PersistedMark): boolean {
  return m.kind === 'review_later' || m.scored_type === 'review_later';
}
...
const TEXT_SELECTION_ACTIONS = new Set(['highlight', 'underline']);

/** 高亮/下划线是文本选择型标注：这些 refs 表示“用户精确选中了哪些字”。 */
export function isTextSelectionMark(m: PersistedMark): boolean {
  if (m.feature_type !== 'markup') return false;
  return TEXT_SELECTION_ACTIONS.has(m.hmp?.action ?? '') || TEXT_SELECTION_ACTIONS.has(m.scored_type ?? '');
}

/** mark 的定位 run 恒等锚并集：reflow_anchor_runs（重排落笔的位置真相）∪ 各笔 anchor_runs ∪ HMP 命中 refs。
 *  这只表示“把笔迹/旁注放回哪里”，不等于“选中了哪些字”。 */
export function markLocationObjectRefs(m: PersistedMark): string[] {
  return [...new Set([
...
/** 文本选择锚点只给高亮/下划线。笔类工具即便靠近正文，也不把正文 refs 当作 selected text。 */
export function markSourceObjectRefs(m: PersistedMark): string[] {
  return isTextSelectionMark(m) ? markLocationObjectRefs(m) : [];
}

function selfContentText(m: PersistedMark): string {
  const hint = (m.hmp?.text_hint ?? '').trim();
  const marked = m.marked_text.trim();
  if (!isTextSelectionMark(m) && hint) return hint;
  if (m.feature_type === 'handwriting') return hint || marked;
  if (m.hmp?.mode === 'self_content') return hint || marked;
  return '';
}
 ...
 *   1) 每条非 folded 的 ai_turn → ai_note（trigger=discussion 时 qa），source 取 ai_turn 自身锚点；只有高亮/下划线锚点才带 quote；
 ...
 *   2) 未被任何 ai_turn 消费的活 mark → excerpt（高亮/下划线）/ annotation（笔/手写/画）；空内容笔不产 KO。
 */
export async function assembleKnowledgeProjection(input: BuilderInput): Promise<KnowledgeProjection> {
  ...
  for (const t of aiTurns) {
    ...
    // 文本 refs 只来自高亮/下划线：笔类工具即便有 reflow/HMP 定位 refs，也不能伪造成“选中了这些字”。
    ...
    const quoteMark = anchorMarks.find(isTextSelectionMark);
    const ko = await finalize({
      ...
      // source 统一取 ai_turn 自身锚点（page/bbox 同源·session 级聚合），quote 仅取文本选择型锚 mark 的所标原文
      pageId: t.page_id,
      pageIndex: t.page_index,
      objectRefs: anchorMarks.length ? anchorObjectRefs : (t.anchor?.object_refs ?? []),
      uriAnchor: anchorObjectRefs[0] ?? anchorIds[0] ?? t.overlay_id,
      bbox: t.overlay?.geometry?.anchor_bbox,
      quote: quoteMark?.marked_text || undefined,
      body,
      provenance: {
  ...
  for (const group of standaloneMarkGroups(marks, consumed)) {
    const m = group[0];
    const reviewLater = group.some(isReviewLaterMark);
    const textAnchored = group.some(isTextSelectionMark);
    let kind: KnowledgeKind = reviewLater ? 'reading_note' : (textAnchored ? 'excerpt' : 'annotation');
    const transcript = group.map(selfContentText).find((t) => !!t);
    const markedText = group.map((x) => x.marked_text.trim()).find((t) => !!t) ?? '';
    // excerpt 正文=所标原文（空则无价值·跳）；annotation（手写/画）正文=识别到的自身内容，
    // **再退回占位**——纯图形/未识别手写也要产 KO，否则用户真画过的圈画在导出里无声消失（与会议侧 inkBody 同口径）。
    const inkBody = (m.feature_type === 'handwriting' ? INK_PLACEHOLDER_HANDWRITING : INK_PLACEHOLDER_DRAWING);
    const body = reviewLater ? (markedText || '稍后处理') : (kind === 'excerpt' ? markedText : transcript || inkBody);
    ...
    const ko = await finalize({
      ...
      objectRefs,
      uriAnchor: objectRefs[0] ?? m.mark_id,
      bbox,
      quote: textAnchored ? markedText || undefined : undefined,
      body,
      provenance: { created_from: 'mark', mark_ids: markIds },
      status: 'export_ready',
      titleOverride: reviewLater ? (markedText || '稍后处理') : undefined,
