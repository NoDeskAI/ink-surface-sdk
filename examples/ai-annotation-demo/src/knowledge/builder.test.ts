...
describe('reading knowledge projection builder', () => {
  it('computes stable SHA-256 when crypto.subtle is unavailable', async () => {
    const hash = await withoutSubtle(() => sha256HexStr('abc'));
  ...
  it('highlight and underline are text-anchored excerpts with exact source refs', async () => {
    const highlight = mark({
      ...
      marked_text: '高亮选中的原文',
      hmp: hmp({ action: 'highlight', target_object_refs: ['run_hi_1', 'run_hi_2'] }),
    });
    const underline = mark({
      mark_id: 'm-under',
      seq: 2,
      created_at: '2026-06-26T06:35:11.000Z',
      feature_type: 'markup',
      scored_type: 'underline',
      marked_text: '下划线选中的原文',
      hmp: hmp({ action: 'underline', target_object_refs: ['run_under_1'] }),
    });

    const kos = await assembleKnowledgeObjects(input([highlight, underline], []));

    expect(kos.map((ko) => ko.kind)).toEqual(['excerpt', 'excerpt']);
    expect(kos[0]).toMatchObject({
      body_md: '高亮选中的原文',
      source: { quote: '高亮选中的原文', object_refs: ['run_hi_1', 'run_hi_2'] },
    });
    expect(kos[1]).toMatchObject({
      body_md: '下划线选中的原文',
      source: { quote: '下划线选中的原文', object_refs: ['run_under_1'] },
    });
  });

  it('pen-like marks do not become text-selected excerpts even when they have nearby text refs', async () => {
    const m = mark({
      mark_id: 'm-pen-circle',
      feature_type: 'markup',
      scored_type: 'circle',
      marked_text: '这段只是附近正文，不是被精确选中的文字',
      hmp: hmp({ action: 'enclosure', target_object_refs: ['run_nearby_1'], mode: 'anchored' }),
    });

    const kos = await assembleKnowledgeObjects(input([m], []));

    expect(kos).toHaveLength(1);
    expect(kos[0].kind).toBe('annotation');
    expect(kos[0].body_md).toBe('（图形标注 / 圈画）');
    expect(kos[0].source.quote).toBeUndefined();
    expect(kos[0].source.object_refs).toEqual([]);
    expect(kos[0].source.inkloop_uri).toBe('inkloop://doc/doc_test/page/13?anchor=m-pen-circle');
  });
  ...
  it('standalone handwriting mark → annotation, body from text_hint', async () => {
    ...
    expect(ko.source.object_refs).toEqual([]);
    expect(ko.tags).toEqual(['inkloop', 'inkloop/annotation']);
  });

  it('explicit reading action cues stay reading annotations instead of meeting objects', async () => {
    const task = mark({ mark_id: 'm-task', created_at: '2026-06-26T06:35:10.000Z', feature_type: 'handwriting', hmp: hmp({ action: 'handwriting', text_hint: '任务：补齐 M103 手写延迟验收数据' }) });
    const decision = mark({ mark_id: 'm-decision', seq: 2, created_at: '2026-06-26T06:35:11.000Z', feature_type: 'handwriting', hmp: hmp({ action: 'handwriting', text_hint: '决策：V1 演示固定为 Web 导入到墨水屏标记再到 Obsidian 投影' }) });
    const risk = mark({ mark_id: 'm-risk', seq: 3, created_at: '2026-06-26T06:35:12.000Z', feature_type: 'handwriting', hmp: hmp({ action: 'handwriting', text_hint: '风险：T10C Plus 低延迟方案还未验证' }) });
    const kos = await assembleKnowledgeObjects(input([task, decision, risk], []));

    expect(kos.map((ko) => ko.kind)).toEqual(['annotation', 'annotation', 'annotation']);
    expect(kos.map((ko) => ko.body_md)).toEqual([
      '任务：补齐 M103 手写延迟验收数据',
      '决策：V1 演示固定为 Web 导入到墨水屏标记再到 Obsidian 投影',
      '风险：T10C Plus 低延迟方案还未验证',
    ]);
    expect(kos.map((ko) => ko.render_hints?.markdown_callout)).toEqual(['note', 'note', 'note']);
    expect(kos.map((ko) => ko.tags[1])).toEqual(['inkloop/annotation', 'inkloop/annotation', 'inkloop/annotation']);
  });

  it('review-later mark becomes a visible Reading Note instead of a page-title placeholder', async () => {
    const m = mark({
      mark_id: 'm-review-later',
      feature_type: 'drawing',
      kind: 'review_later',
      scored_type: 'review_later',
      marked_text: '稍后处理',
      ai_eligible: false,
      strokes: [{ tool: 'pen', points: [{ x: 0.9, y: 0.1, t: 0, pressure: 0.6 }] }],
      hmp: null,
    });
    const kos = await assembleKnowledgeObjects(input([m], []));

    expect(kos).toHaveLength(1);
    expect(kos[0].kind).toBe('reading_note');
    expect(kos[0].title).toBe('稍后处理');
    expect(kos[0].body_md).toBe('稍后处理');
    expect(kos[0].render_hints?.markdown_callout).toBe('note');
    expect(kos[0].tags).toEqual(['inkloop', 'inkloop/reading-note']);
  });
