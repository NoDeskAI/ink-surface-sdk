...
object HqHwBridge {
    ...
    private const val CODE_OPEN_PEN_DOWN_ENABLE = 4010 // 原厂 HWTool.automaticallyDrawOpen/Close 也会同步开关
    ...
    @Volatile private var socketConnectCount = 0L
    @Volatile private var socketPacketCount = 0L
    @Volatile private var socketNonStylusDropCount = 0L
    @Volatile private var socketOutOfAreaDropCount = 0L
    @Volatile private var socketDeliveredStrokeCount = 0L
    @Volatile private var socketLastError = ""
    @Volatile private var socketLastPacket = ""
    @Volatile private var socketLastDelivery = ""
    @Volatile private var lastArmAtMs = 0L
    @Volatile private var lastDrawEnableReply = Int.MIN_VALUE
    @Volatile private var lastPenDownEnableOk = false
    @Volatile private var lastPenDownDisableOk = false
    @Volatile private var lastSocketEventsEnableOk = false
    @Volatile private var lastSocketEventsDisableOk = false
    @Volatile private var lastNativeAreaText = ""
    @Volatile private var lastViewportAreaText = ""
    @Volatile private var clearFastInkCount = 0L
    @Volatile private var resetFastInkCount = 0L
    @Volatile private var lastClearFastInkAtMs = 0L
    @Volatile private var lastResetFastInkAtMs = 0L
    ...
    fun clearFastInk() {
        ...
        if (index < 0) return
        clearFastInkCount += 1
        lastClearFastInkAtMs = android.os.SystemClock.uptimeMillis()
        if (!transactBool(CODE_CLEAR_INDEX, index)) transactNoReply(CODE_CLEAR_ALL, 2)
    }

    /** 明确离开可写面时调用：清掉硬件 OSD 临时墨迹，并解除当前画区。
     *  OSD 是物理屏叠加层，不随 WebView 页面刷新；如果只切到书架但不解除画区，旧笔迹会固定盖在新页面上。 */
    @JvmStatic
    fun resetFastInk() {
        val index = activeAreaIndex
        if (!armed && index < 0) return
        resetFastInkInternal(index, force = false, reason = "reset")
    }

    /** 强制视觉边界清理：用于启动、页面刷新、宿主销毁这类可能已经丢失本进程 armed/index 状态的场景。
     *  drawserver 是独立守护进程，force-stop/reload 后旧 OSD 画区和墨迹可能仍留在硬件叠加层里；普通
     *  resetFastInk 为了避免空闲态反复清屏会在无状态时 no-op，所以这里必须单独提供强清入口。 */
    @JvmStatic
    fun forceResetFastInk() {
        resetFastInkInternal(activeAreaIndex, force = true, reason = "force")
    }

    @Synchronized
    private fun resetFastInkInternal(index: Int, force: Boolean, reason: String) {
        resetFastInkCount += 1
        lastResetFastInkAtMs = android.os.SystemClock.uptimeMillis()
        try {
            if (index >= 0) {
                if (!transactBool(CODE_CLEAR_INDEX, index)) transactNoReply(CODE_CLEAR_ALL, 2)
                transactNoReply(CODE_REMOVE_DRAW_AREA, index)
            } else if (force) {
                clearStaleDrawAreas()
            ...
            Log.w(TAG, "resetFastInk clear failed: ${t.javaClass.simpleName}: ${t.message}")
        } finally {
            disarm(force)
            Log.i(TAG, "resetFastInk count=$resetFastInkCount reason=$reason force=$force")
        }
    }

    @JvmStatic
    fun resetDiagnostics() {
        socketPacketCount = 0L
        socketNonStylusDropCount = 0L
        socketOutOfAreaDropCount = 0L
        socketDeliveredStrokeCount = 0L
        socketLastError = ""
        socketLastPacket = ""
        socketLastDelivery = ""
        resetSocketStroke()
    ...
    fun onWindowFocusChanged(has: Boolean) {
        hasFocus = has
        if (has) webViewRef?.get()?.post { tryArmFromReported() } else resetFastInk()
    }
    ...
    private fun updateReportedArea(rect: Rect?) {
        reportedRect = rect
        if (!hasFocus) return
        if (rect == null || rect.isEmpty) { resetFastInk(); return }
        ...
        lastNativeAreaText = native.toShortString()
        lastViewportAreaText = rect.toShortString()
    }
    ...
    private fun arm(view: View, viewportRect: Rect) {
        ...
        try {
            ...
            lastArmAtMs = android.os.SystemClock.uptimeMillis()
            lastDrawEnableReply = ret
            lastNativeAreaText = nativeArea.toShortString()
            lastViewportAreaText = viewportRect.toShortString()
            Log.i(TAG, "armed index=$index viewportRect=$viewportRect nativeArea=$nativeArea drawEnableReply=$ret")
            // 对齐原厂 HWTool.automaticallyDrawOpen：先允许 pen-down，再开 native 笔点广播。
            lastPenDownEnableOk = setPenDownEnabled(true)
            // 开 native 笔点广播 + 起 socket 读取线程：抬笔时用同源硬件点补画，消除微重影。
            lastSocketEventsEnableOk = setSocketEventsEnabled(true)
    ...
    @Synchronized
    private fun disarm(force: Boolean = false) {
        if (!force && !armed && cached == null) return
        stopUnifiedSocketReader()
        lastSocketEventsDisableOk = setSocketEventsEnabled(false)
        lastPenDownDisableOk = setPenDownEnabled(false)
        try {
            val index = activeAreaIndex
            ...
            Log.i(TAG, "disarmed index=$index")
        } catch (t: Throwable) {
    ...
    /** 4010：原厂 HWTool 常量名为 AUTOMATICALLY_DRAW / IS_OPEN_PEN_DOWN_ENABLE。它是 no-reply 事务，
     *  只能记录 transact 是否被服务接受，真实效果由 event3/socket/持久 mark 对照验证。 */
    private fun setPenDownEnabled(enable: Boolean): Boolean {
        val b = service() ?: return false
        val data = Parcel.obtain()
        return try {
            data.writeInt(if (enable) 1 else 0)
            b.transact(CODE_OPEN_PEN_DOWN_ENABLE, data, null, 0)
        } catch (t: Throwable) {
            Log.w(TAG, "4010(${if (enable) 1 else 0}) failed: ${t.javaClass.simpleName}: ${t.message}")
            false
        } finally {
            data.recycle()
        }
        ...
        socketThread = kotlin.concurrent.thread(name = "InkLoop-HqUnifiedSocket", isDaemon = true) {
            ...
            while (socketRun) {
                try {
                    ...
                    socketLastError = ""
                    Log.i(TAG, "unified socket connected count=$socketConnectCount name=$UNIFIED_SOCKET")
                    val input = s.inputStream
                    while (socketRun && readFully40(input, buf)) handleSocketPacket(buf)
                } catch (t: Throwable) {
                    socketLastError = "${t.javaClass.simpleName}: ${t.message}"
                    if (socketRun) Log.w(TAG, "unified socket loop: $socketLastError")
                } finally {
    ...
    private fun handleSocketPacket(bytes: ByteArray) {
        ...
        socketPacketCount += 1
        socketLastPacket = "action=${p.action} flag=${p.flag} native=(${p.nativeX},${p.nativeY}) pressure=${p.pressureRaw} width=${p.strokeWidth}"
        if (socketPacketCount <= 8 || p.action == A_DOWN || p.action == A_UP || p.action == A_CANCEL) {
            Log.d(TAG, "unified socket packet #$socketPacketCount $socketLastPacket")
        }
        ...
        if (p.flag != TOOL_STYLUS) { // 非笔尖(橡皮/手指)不喂持久层——橡皮走 WebView eraseAt
            socketNonStylusDropCount += 1
            if (socketNonStylusDropCount <= 8 || p.action == A_UP || p.action == A_CANCEL) {
                Log.d(TAG, "drop non-stylus packet count=$socketNonStylusDropCount action=${p.action} flag=${p.flag}")
            }
        ...
        if (!pointInReportedArea(css)) { // 出画区：把已缓冲的这一笔收尾交付，之后的点丢弃
            socketOutOfAreaDropCount += 1
            if (socketOutOfAreaDropCount <= 8 || p.action == A_UP || p.action == A_CANCEL) {
                Log.d(TAG, "drop out-of-area count=$socketOutOfAreaDropCount css=(${css.x},${css.y}) area=$reportedRect")
            }
    ...
    private fun finishSocketStroke() {
        ...
        socketDeliveredStrokeCount += 1
        socketLastDelivery = "seq=$socketSeq points=${pts.size}"
        Log.i(TAG, "deliver unified socket stroke $socketLastDelivery")
        val view = webViewRef?.get() ?: return
        view.post {
    ...
    private class HqHwJs {
        ...
        @JavascriptInterface
        fun resetFastInk() { HqHwBridge.resetFastInk() }

        @JavascriptInterface
        fun forceResetFastInk() { HqHwBridge.forceResetFastInk() }

        @JavascriptInterface
        fun debugStatus(): String { return HqHwBridge.debugStatusJson() }

        @JavascriptInterface
        fun resetDiagnostics() { HqHwBridge.resetDiagnostics() }
    }
    ...
    private fun debugStatusJson(): String {
        ...
        return try {
            o.put("armed", armed)
                .put("has_focus", hasFocus)
                .put("active_area_index", activeAreaIndex)
                .put("reported_rect", reportedRect?.toShortString() ?: JSONObject.NULL)
                .put("reported_dpr", reportedDpr.toDouble())
                .put("last_arm_at_ms", lastArmAtMs)
                .put("last_draw_enable_reply", lastDrawEnableReply)
                .put("last_pen_down_enable_ok", lastPenDownEnableOk)
                .put("last_pen_down_disable_ok", lastPenDownDisableOk)
                .put("last_socket_events_enable_ok", lastSocketEventsEnableOk)
                .put("last_socket_events_disable_ok", lastSocketEventsDisableOk)
                .put("last_native_area", lastNativeAreaText)
                .put("last_viewport_area", lastViewportAreaText)
                .put("clear_fast_ink_count", clearFastInkCount)
                .put("reset_fast_ink_count", resetFastInkCount)
                .put("last_clear_fast_ink_at_ms", lastClearFastInkAtMs)
                .put("last_reset_fast_ink_at_ms", lastResetFastInkAtMs)
                .put("socket_run", socketRun)
                .put("socket_connected", socketClient != null)
                .put("socket_connect_count", socketConnectCount)
                .put("socket_packet_count", socketPacketCount)
                .put("socket_non_stylus_drop_count", socketNonStylusDropCount)
                .put("socket_out_of_area_drop_count", socketOutOfAreaDropCount)
                .put("socket_delivered_stroke_count", socketDeliveredStrokeCount)
                .put("stroke_buffer_count", strokeBuf.size)
                .put("last_error", socketLastError)
                .put("last_packet", socketLastPacket)
                .put("last_delivery", socketLastDelivery)
            o.toString()
        } catch (_: Throwable) {
            "{}"
        }
