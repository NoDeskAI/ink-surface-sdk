...
import { PanelVaultGuardError, assertNonEmptyVaultRelease, guardPanelVaultRest, guardPanelVaultReqUrl, resolvePanelVaultGuardUser } from './panel-vault-guard';
...
describe('resolvePanelVaultGuardUser · session namespace first', () => {
  it('默认使用当前设备 session user，避免旧 INKLOOP_USER_ID 把飞书用户误判为越桶', () => {
    expect(resolvePanelVaultGuardUser('feishu_ou_demo', '')).toBe('feishu_ou_demo');
  });
  it('显式 force user 仍可钉死单用户 vault 桶', () => {
    expect(resolvePanelVaultGuardUser('feishu_ou_demo', 'local_demo')).toBe('local_demo');
  });
});
