# InkLoop AI Pen V1 Demo Host

这个目录现在是 InkLoop AI Pen Kickstarter V1 的 Web/Desktop 验证宿主。首要演示面是 **AI Pen + Capture Surface + Live Board + InkGraph + Education/Meeting outputs**；原来的 PDF 标注阅读闭环保留为文档阅读和 InkLoop Paper 复用能力，不再是 2026 年 10 月 Kickstarter 基础档的主产品叙事。

## V1 Demo Entry Points
...
日常 Web/WebView/InkLoop Paper ↔ Obsidian 同步走 Runtime Sync，不走整包 vault release。AI Pen V1 的主证据链在同步之前先落为 `RawPenFrame -> Stroke -> InkEvent -> BoardGraph/InkGraph -> LessonGraph/MeetingGraph -> KnowledgeObject`。开发服务器默认提供本地事件日志端点：
...
不用打开 Obsidian 也可以先跑确定性闭环：

```bash
npm run smoke:runtime-sync-flow
```

期望输出里 `ok` 为 `true`，并且 `release_path_used` 为 `false`。

产品级本地 E2E 可以跑：

```bash
npm run smoke:v1-product-e2e
```

这条命令会打当前本机 `http://127.0.0.1:8731` Cloud Hub，验证 Web/电脑端外部导入 cloud-first、Library manifest/SSE、源文件 blob 下载、Runtime event push/pull、LLM 分类器、Obsidian Markdown 投影、AI Graph worker 后处理边界，以及 Lark Meeting Timeline SDK 的会议事件标记链路。输出的阅读投影写到 `test-results/v1-product-e2e-vault`，会议投影写到 `test-results/meeting-v1-e2e-vault`。

这条链路同时要求 Cloud Hub 暴露持久化后处理 API：

```text
POST/GET /v1/knowledge/ai-turns
POST/GET /v1/knowledge/objects
POST/GET /v1/knowledge/document-projections
```

推荐的 V1 产品级验收入口是：

```bash
npm run verify:v1-product
```

这条命令不依赖真机，覆盖 TypeScript 检查、Runtime Sync cursor、Cloud Hub session/device、用户隔离、Obsidian 受控回写、Web 导入后处理持久化、Reading/Meeting Obsidian 投影。

如果要带跑 M103 真机，先确认 Cloud Hub 固定在 `8731`，设备已通过 `adb` 连接，然后运行：

```bash
npm run verify:v1-product:paper
```

这条命令在无设备验收基础上增加 M103 Web/Cloud-first 导入、Paper Wi-Fi/LAN local-first 导入、Paper Library 自动出现、下载本地可读、设备 WebView 写 Mark、重开同一本书确认 Mark 不丢、Runtime Sync 后处理、Obsidian Reading 投影。

需要只复跑书架/标记路径时，用：

```bash
npm run smoke:m103-cloud-library-device
```

这条命令验证 Web/Cloud 上传的新源文件能在 Paper Library 自动出现、下载成本地可读文档、打开为 Markdown 阅读页，并在设备 WebView 内写入一条 Mark 后重开同一本书确认 Mark 没有丢。

M103 低延迟笔迹现在分两层验证：`npm run smoke:m103-physical-pen-synthetic` 只证明 `/dev/input/event3 -> HqHw socket -> RawPenFrame -> JSONL` 下游链路可用；真实手写验收仍需要 `npm run smoke:m103-stylus-input:live` 期间在设备上画一笔，并观察是否产生真实 Huion 事件。

---

## 核心交互

下面是保留下来的源文档阅读验证能力。Kickstarter V1 演示请优先打开 `ai-pen-demo.html`。
...
## 历史源文档阅读盘点（截至 2026-06-13）

下面内容记录的是 6 月 PDF/源文档阅读宿主的历史验证状态，用于保留阅读、标记、重排和推理接缝的上下文。它不是 2026 年 10 月 Kickstarter V1 基础档的产品承诺；当前主线以 AI Pen + Capture Surface + Education/Meeting outputs 为准。
