...
- SDK 包目录：`packages/ink-surface-sdk`
- 兼容构建产物：`packages/ink-surface-sdk/dist/inkloop-surface-sdk.*.js`
...
详细 SDK 使用文档见 [docs/ink-surface-sdk.md](./docs/ink-surface-sdk.md)，正式架构文档见 [docs/architecture.md](./docs/architecture.md)。
...
本仓库同时保留 InkLoop Web demo 和 Obsidian Runtime MVP。它们是 SDK 的 examples/宿主验证，不是 SDK 内部实现；demo 通过 `ink-surface-sdk` 包名消费 `packages/ink-surface-sdk`：
